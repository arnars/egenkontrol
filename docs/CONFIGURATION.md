...
`config/rengoering.demo.json`, `config/personlig-hygiejne.demo.json`, `config/adskillelse.demo.json`, `config/skadedyr.demo.json` og `config/varemodtagelse.demo.json` driver arbejdsgangsvisningerne. Alle er udkast. Planernes områder, processer, ansvar og frekvenser skal tilpasses den faktiske drift og indretning. Varemodtagelsesfilen konfigurerer kontrolpunkter, fejltyper og mulige handlinger, men fastlægger ikke en normal dokumentationshyppighed. JSON Schema og Zod-validering tilføjes, når formen har været stabil gennem frontenditerationerne.
